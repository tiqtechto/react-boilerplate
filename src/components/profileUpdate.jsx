...
const ProfileUpdate = () => {
    ...
    const onSubmitPassword = (data) => {
        if(isObjEmpty(errors)){
            setBTNDisable(true);
            execute('/password-update', 'post', data);
        } else {
            setBTNDisable(false);
        }
            ...
            if(errorf !== null){
                ...
                if(responsef !== null){
                    if(responsef.status === 200){
                        setUserData({firstName: responsef.data?.firstName, lastName: responsef.data?.lastName, email: responsef.data?.email, verified: responsef.data?.verified, image: responsef.data?.image.contentType+','+responsef.data?.image.data});
                    }
                }
            }  
        },[errorf, responsef]);
